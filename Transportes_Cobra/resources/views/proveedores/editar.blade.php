...
@section('content')
<form method="POST" action="{{route('updateproveedor')}}" id="form-edit-prov" accept-charset="UTF-8" enctype="multipart/form-data">
@csrf
    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                <input type="hidden" name="proveedor_id" id="proveedor_id" value="{{ $datos['id'] }}"/>
                    <div class="row">
                        <div class="col-md-6">
                            <label for="nombreprov">Proveedor:</label>
                            <input tipe="text" class="form-control lg-4 campo-requerido{{ $errors->has('nombreprov') ? ' is-invalid' : '' }}" value="{{ !empty(old('nombreprov')) ? old('nombreprov') : $datos['nombre'] }}" id="nombreprov" name="nombreprov">
                        ...
                        <div class="col-md-6">
                            ...
                            <input tipe="text" class="form-control lg-4" id="descripcion" name="descripcion" value="{{ !empty(old('descripcion')) ? old('descripcion') : $datos['descripcion'] }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-5">
                        ...
                        <div class="col-md-1">
                            <label>&nbsp;</label>
                            <input type="hidden" class="campo-requerido" name="hiddenTipoServicios" id="hiddenTipoServicios" value="{{ $idcont['id_control'] }}"/>
                            <button id="addservicios" type="button" class="form-control btn btn-primary btn-add" disabled><i class="fas fa-plus"></i></button>
                        </div>   
                    </div>
                    <div class="form-group row">
                        <div class="col-md-6">
                            <div id="content_list_tiposervicio" class="col-md-12">
                                <strong>Lista de módulos padre agregados</strong> <input type="button" class="quitarlista btn btn-danger btn-sm" data-lista="#list_tiposervicio" data-afectado="#hiddenTipoServicios" data-tipo="checkremove_servicio" value="Quitar de la lista" />
                            </div>

                            <div class="col-md-12">
                                <ul id="list_tiposervicio">
                                @foreach($servicios as $val)
                                    <li id="elementoLista_{{ $val['tipo_servicio_id']}}">{{ $val['nombre']}}<input type="checkbox" value="{{ $val['tipo_servicio_id']}}" class="checkremove_servicio"/></li>
                                @endforeach
                                </ul>
                            </div>
                        </div>
...
$('#enviar').click(function() {
    swal.fire({
        ...
        if (result.value) {
            $('#form-edit-prov').submit();
        }
    });
});
$('#form-edit-prov').validate({
    ignore: "",
